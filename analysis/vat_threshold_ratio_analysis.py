...
def analyze_vat_threshold_bunching():
    ...
    for sector in sorted(df['sector'].unique()):
        ...
        if firms_above > 0:
            ...
            ratio = np.inf if firms_below > 0 else 0
    ...
    results_df = results_df[results_df['bunching_ratio'] != np.inf]  # Remove infinite ratios
    results_df = results_df.sort_values('bunching_ratio', ascending=False)
    
    # Save full results
    output_path = Path(__file__).parent / 'vat_threshold_bunching_by_sector.csv'
    results_df.to_csv(output_path, index=False)
    
    # Print ranked output
    print("\n" + "="*100)
    print("VAT THRESHOLD BUNCHING ANALYSIS BY SECTOR")
    ...
    # Sectors with strongest bunching (ratio > 2)
